...
void test_ragfile_create_save_load() {
    ...
    assert(ragfile_create(&rf, tokens, 8, embedding, 4, metadata, 
                          ragfile_compute_id_hash("test_tokenizer"),
                          ragfile_compute_id_hash("test_embedding"), 256) == RAGFILE_SUCCESS);
    ...
    float similarity = jaccard_similarity(rf->minhash, loaded_rf->minhash);
